...
void psgplay_set_digital_lowpass_length (struct psgplay *pp, int length)
{
	...
	/* if we shrank and the current-position is outside the window buffer, yank it back to the start */
	if (pp->downsample.lowpass.left.k >= pp->downsample.lowpass.left.length)
	{
